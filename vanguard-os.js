const VanguardOS = (() => {
    ...
    const renderApps = () => {
        ...
        if (!dashboard) return;
        dashboard.innerHTML = ''; // Clear previous
        appsConfig.forEach(app => {
            const appElement = document.createElement('a');
            appElement.className = 'app-icon';
            appElement.href = app.path.startsWith('http') ? '#' : app.path;
            if (app.path.startsWith('http')) {
                appElement.onclick = (e) => { e.preventDefault(); openSocial(app.path); };
            }
            appElement.innerHTML = `<div class="icon-box"><svg class="glyph" viewBox="0 0 24 24" width="32" height="32" fill="white"><path d="${app.glyph}"/></svg></div><span>${app.name}</span>`;
    ...
    const openSocial = (url) => {
        ...
        if (modal) { modal.style.display = 'flex'; document.getElementById('modal-link').href = url; }
    };

    return { 
        init: () => { setupStarfield(); renderApps(); },
        openBlogModal: (title, content, id) => {
